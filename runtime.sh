...
FINALVER=$(curl http://admin:admin123@10.128.0.7:8081/service/rest/repository/browse/studenapp-snapshot/com/jdevs/studentapp/$VER/ | html2text | grep "$IDNO-" | tail -1 | awk -F  - '{print $1F}')
http_url=http://admin:admin123@10.128.0.7:8081/repository/studenapp-snapshot/com/jdevs/studentapp
STUDENTWAR=$http_url/$VER/studentapp-$FINALVERabc
echo $VER
echo $IDNO
echo $FINALVER
echo $http_url
